...
function LearnPageContent() {
    ...
    // Default to TRUE so chapter 2+ can be generated by LLM
    const enableDynamicGeneration = process.env.NEXT_PUBLIC_ENABLE_DYNAMIC_GENERATION !== 'false';
    ...
    useEffect(() => {
        ...
        if (courseId && !enableDynamicGeneration) {
            apiClient.getCourse(courseId).then(fetchedCourse => {
                // Filter to only include chapter_1 (baseline chapter)
                // All other chapters should use dynamic generation when available
                const chapter1Only = {
                    ...fetchedCourse,
                    chapters: fetchedCourse.chapters.filter(ch => ch.id === "chapter_1")
                };
                setCourse(chapter1Only);
            });
