...
class ScreenTimeTracker:
    ...
    def run(self, callback=None):
        ...
        Args:
            callback: Optional function to call with updates (is_focused, score)
        """
        if not self.cap.isOpened():
            self.cap = cv2.VideoCapture(0)
            if not self.cap.isOpened():
        ...
        print("Screen Time Tracker Initialized")
        ...
        try:
            while self.is_running:
                ...
                if callback:
                    # Calculate current focus score (simple metric for now)
                    score = 1.0 if self.is_looking else 0.0
                    callback(self.is_looking, score)
...
if __name__ == "__main__":
    ...
    # For testing, we just run it and stop manually with Ctrl+C
    try:
        tracker.run()
